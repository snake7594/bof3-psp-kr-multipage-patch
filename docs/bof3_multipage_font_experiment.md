...
`ENDKANJI.EMI`를 확장해 441칸 페이지 3개, 총 1,323칸을 확보했고, 현재 번역에서 필요한 한글 1,147자를 모두 배치했습니다. `FIRST.EMI`와 `BATL_RET.EMI`는 시작 직후 크래시를 피하기 위해 새 섹션을 추가하지 않고, 원래 존재하는 보조 폰트 페이지 안에서만 한글 글리프를 교체합니다.
...
- `BOOT.BIN`: 패치 적용
- `EBOOT.BIN`: 패치 적용
- `FIRST.EMI`: 원본 크기 유지, 기존 보조 폰트 페이지 2개 교체
- `BATL_RET.EMI`: 원본 크기 유지, 기존 보조 폰트 페이지 1개 교체
...
- `PSP_GAME/USRDIR/JPN/ETC/FIRST.EMI`
  - 크기: 329,728 bytes
  - SHA-256: `A2E9A3643B5AB87EDD75C0480BD8D43A2B9300AD9AFD8A0D668DE33FF9F891AB`
- `PSP_GAME/USRDIR/JPN/BATTLE/BATL_RET.EMI`
  - 크기: 98,304 bytes
  - SHA-256: `AF21A1E696540D8FD78072B0C781CF7B3CC0D5BEF351CFA112B8C0193186FC19`
